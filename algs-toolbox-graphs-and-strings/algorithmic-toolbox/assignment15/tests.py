...
class LCS2(unittest.TestCase):
    ...
    def test_stress(self):
        ...
        columns = 50
        rows = 10000
        random_sequences = [random.choices(range(low, high),
                            k=random.randrange(columns)) for _ in range(rows)]
        for i in range(0, rows, 2):
            first_sequence = random_sequences[i]
            second_sequence = random_sequences[i+1]
            self.assertEqual(lcs2(first_sequence, second_sequence),
                             reference(first_sequence, second_sequence))
